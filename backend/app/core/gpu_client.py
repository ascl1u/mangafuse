...
class RunpodGpuClient:
    """Client for invoking a Runpod serverless endpoint to run the pipeline."""

    def __init__(self, *, api_key: str, endpoint_id: str, public_backend_url: str, callback_secret: Optional[str]) -> None:
    ...
    def submit_job(
        ...
        *,
        ...
        # Build handler-compatible body (same as local service), include callback
        callback_url = f"{self._public_backend}/api/v1/gpu/callback"
        body = {
            "job_id": job_id,
            "mode": mode,
        ...
        payload = json.dumps({
            ...
            "userProvidedId": f"mf-{job_id}",
        }).encode("utf-8")

        # Basic retry with backoff and jitter
        max_attempts = int(os.getenv("GPU_SUBMIT_MAX_RETRIES", "3"))
        base_backoff_ms = int(os.getenv("GPU_SUBMIT_INITIAL_BACKOFF_MS", "500"))
        jitter_ms = int(os.getenv("GPU_SUBMIT_JITTER_MS", "200"))
        ...
        while True:
            ...
            try:
                with httpx.Client(timeout=15.0) as client:
                    resp = client.post(
                        f"https://api.runpod.ai/v2/{self._endpoint_id}/run",
                        content=payload,
                ...
                status_code = exc.response.status_code
                if 400 <= status_code < 500:
                    logger.error("runpod_submit_client_error", extra={"status": status_code, "job_id": job_id})
                    raise RuntimeError(f"gpu submit failed: {exc}") from exc
            except (httpx.TimeoutException, httpx.NetworkError, httpx.TransportError) as exc:  # type: ignore[attr-defined]
                pass

            if attempt >= max_attempts:
                logger.error("runpod_submit_failed", extra={"attempts": attempt, "job_id": job_id})
                raise RuntimeError("gpu submit failed after retries")
...
def get_gpu_client() -> GpuClient:
    ...
    if settings.gpu_service_provider == "runpod":
        if not (settings.runpod_api_key and settings.runpod_endpoint_id and settings.public_backend_base_url):
            raise RuntimeError("Runpod client requires RUNPOD_API_KEY, RUNPOD_ENDPOINT_ID, and PUBLIC_BACKEND_BASE_URL")
        return RunpodGpuClient(
